...
describe('GET /api/licenses', () => {
    it('returns 401 without auth', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/licenses');
        expect(res.status).toBe(401);
    });
...
describe('GET /api/licenses/:id', () => {
    it('returns 401 without auth', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get(`/api/licenses/${licenseId}`);
        expect(res.status).toBe(401);
    });
...
//# sourceMappingURL=licenses.test.js.map