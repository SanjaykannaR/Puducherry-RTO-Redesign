...
describe('GET /api/vehicles', () => {
    it('returns 401 without auth', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/vehicles');
        expect(res.status).toBe(401);
    });
...
describe('GET /api/vehicles/search/:regNo', () => {
    it('returns 404 for non-existent vehicle', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/api/vehicles/search/NONEXISTENT');
        expect(res.status).toBe(404);
    });
    it('finds vehicle by registration number', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get(`/api/vehicles/search/${encodeURIComponent(testRegNo)}`);
        expect(res.status).toBe(200);
        expect(res.body.registrationNo).toBe(testRegNo);
        expect(res.body.make).toBe('Honda');
    });
...
describe('GET /api/vehicles/:id', () => {
    it('returns 401 without auth', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get(`/api/vehicles/${vehicleId}`);
        expect(res.status).toBe(401);
    });
